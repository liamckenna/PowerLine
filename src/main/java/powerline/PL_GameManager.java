...
public class PL_GameManager implements Listener {
    ...
    public void AddMember(Vector<Player> team, Player player) {
        if (team_a_members.contains(player))
            team_a_members.remove(player);
        if (team_b_members.contains(player))
            team_b_members.remove(player);
        if (spectator_members.contains(player))
            spectator_members.remove(player);
        team.add(player);
        String player_name = player.getName();
        if (team == team_a_members)
            Bukkit.broadcast(text(player_name + " has joined Team Emerald!"));
        else if (team == team_b_members)
            Bukkit.broadcast(text(player_name + " has joined Team Lapis!"));
        else if (team == spectator_members)
            Bukkit.broadcast(text(player_name + " is now spectating!"));
    ...
    public void ResetMatch() {
        ...
        if (old_match.map.team_a.status == PL_TeamStatus.LOST)
            team_b_wins++;
        else if (old_match.map.team_b.status == PL_TeamStatus.LOST)
            team_a_wins++;
    ...
    public static void CacheResources() {
        ...
        PL_Resource.plr_potions
                .AddDrop(new PL_Drop(PL_Helpers.CreatePotion(Material.POTION, PotionType.SPEED, false, false), 1));
        PL_Resource.plr_potions
                .AddDrop(new PL_Drop(PL_Helpers.CreatePotion(Material.POTION, PotionType.STRENGTH, false, false), 1));
        PL_Resource.plr_potions.AddDrop(
                new PL_Drop(PL_Helpers.CreatePotion(Material.POTION, PotionType.INVISIBILITY, false, false), 1));
        PL_Resource.plr_potions.AddDrop(
                new PL_Drop(PL_Helpers.CreatePotion(Material.POTION, PotionType.INSTANT_HEAL, false, false), 1));
        PL_Resource.plr_potions
                .AddDrop(new PL_Drop(PL_Helpers.CreatePotion(Material.POTION, PotionType.REGEN, false, false), 1));
        PL_Resource.plr_potions.AddDrop(
                new PL_Drop(PL_Helpers.CreatePotion(Material.SPLASH_POTION, PotionType.INSTANT_HEAL, false, false), 1));
        PL_Resource.plr_potions.AddDrop(new PL_Drop(
                PL_Helpers.CreatePotion(Material.SPLASH_POTION, PotionType.INSTANT_DAMAGE, false, false), 1));
        PL_Resource.plr_potions.AddDrop(
                new PL_Drop(PL_Helpers.CreatePotion(Material.SPLASH_POTION, PotionType.POISON, false, false), 1));
        PL_Resource.plr_potions.AddDrop(
                new PL_Drop(PL_Helpers.CreatePotion(Material.SPLASH_POTION, PotionType.SLOWNESS, false, false), 1));
        PL_Resource.plr_potions.AddDrop(
                new PL_Drop(PL_Helpers.CreatePotion(Material.SPLASH_POTION, PotionType.WEAKNESS, false, false), 1));
